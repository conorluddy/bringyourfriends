...
class OmniAuthApp < Sinatra::Base
    ...
    get '/trip/:tripid' do
      ...
      endmarker = '&markers=color:white%7Clabel:B%7C'   + @endLat.to_s   + ',' + @endLon.to_s
      
      markers = startmarker + endmarker
      path = '&path=color:0x0000ff|weight:5|' + @startLat.to_s + ',' + @startLon.to_s + '|' + @endLat.to_s   + ',' + @endLon.to_s

      @mapsrc = 'http://maps.googleapis.com/maps/api/staticmap?&size=600x300' + markers + path + '&key=AIzaSyA94pcuOlREd4FULaI6eI5KlbBCUFDuCDg&sensor=true' 
      # / Map setup
